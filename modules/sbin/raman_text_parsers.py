...
def save_map_sample_file(in_filename, out_filename, prefix):
    ...
    with h5py.File(out_filename, 'w') as out_file:
        out_file.create_dataset('x', data=x.reshape(1, -1))
        out_file.create_dataset('Y', data=Y)
        out_file.create_dataset('spatial_x', data=spatial_x.reshape(-1, 1))
        out_file.create_dataset('spatial_y', data=spatial_y.reshape(-1, 1))
        out_file.attrs['name'] = np.string_(f'{prefix}: {os.path.basename(in_filename)}')
        out_file.attrs['filename'] = np.string_(os.path.basename(in_filename))
