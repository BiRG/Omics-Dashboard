...
def list_sample_groups():
    try:
        ...
        if request.method == 'POST':
            ...
            return jsonify(
                dt.sample_groups.create_sample_group(current_user, new_data).to_dict())
...
def get_sample_group(sample_group_id=None):
    try:
        ...
        if request.method == 'GET':
            return jsonify(
                {**sample_group.to_dict(), 'is_write_permitted': dt.users.is_write_permitted(user, sample_group)})
